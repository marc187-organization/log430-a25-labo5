...
from orders.controllers.order_controller import (
    ...
    remove_order,
    get_order,
    get_report_highest_spending_users,
    get_report_best_selling_products,
    update_order,
)
from orders.controllers.user_controller import create_user, remove_user, get_user
from stocks.controllers.product_controller import (
    create_product,
    remove_product,
    get_product,
)
from stocks.controllers.stock_controller import (
    get_stock,
    populate_redis_on_startup,
    set_stock,
    get_stock_overview,
    update_stock,
)
...
resource = Resource.create(
    {"service.name": "store_manager", "service.version": "1.0.0"}
)
...
def post_orders():
    ...
    with tracer.start_as_current_span("[store_manager] POST /orders"):
...
def delete_orders_id(order_id):
    ...
    with tracer.start_as_current_span("[store_manager] DELETE /orders/{order_id}"):
...
def put_stocks():
    ...
    with tracer.start_as_current_span("[store_manager] PUT /stocks"):
